...
class FileGenerator:
    ...
    def generate(self, structure: ProjectStructure, output_path: Path, 
        ...
        # Generate all items
        self._generate_items(structure.items, root_path, structure.variables, progress_callback)
    
    def _generate_items(self, items: list[StructureItem], base_path: Path, 
                       variables: dict, progress_callback: Optional[Callable] = None):
        ...
        for item in items:
            ...
            if item.item_type == ItemType.DIRECTORY:
                ...
                self._generate_items(item.children, item_path, variables, progress_callback)
